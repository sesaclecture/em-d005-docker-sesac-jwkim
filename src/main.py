...
def make_docker_run_command(image_name):
   return f"docker run -itd {image_name} /bin/bash"
...
def make_port_mapping_command(
    ...
    return f"docker run --rm -p{host_port}:{container_port} {image_name}"
...
def make_volume_mapping_command(
    ...
    return f"docker run -d -v {host_dir}:{container_dir} {image_name}"
